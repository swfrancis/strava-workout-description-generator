...
def test_strava_client(access_token=None):
    ...
    try:
        ...
        print(f"✅ Connected as: {athlete.get('firstname')} {athlete.get('lastname')}")
        ...
        if activities:
            ...
            try:
                ...
                print(f"   Splits: {len(details.get('splits_metric', []))} splits")
                ...
                for i, lap in enumerate(laps[:3], 1):
                    ...
                    print(f"   Lap {i}: {elapsed//60}:{elapsed%60:02d} - {distance/1000:.2f}km")
                
                print(f"\n📈 Testing streams for activity {activity_id}...")
                streams = client.get_activity_streams(activity_id, keys=['time', 'distance', 'velocity_smooth'])
